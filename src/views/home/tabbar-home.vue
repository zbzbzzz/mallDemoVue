<template>
  <div class="tab_home">
    <div class="location" @click="open()">
      <span>
        <van-icon name="weizhi" color="#0066FF" size="25" />
      </span>
      <span>{{addr}}</span>
      <span>
    ...
    <div class="tal_class_searchBox">
      <van-search placeholder="点击前往搜索" @click="$router.push({ name: 'search' })" shape="round" />
      <div class="tal_class_searchMask"></div>
    </div>
    <van-swipe :autoplay="3000" :show-indicators="false" :height="150">
    ...
    <van-grid :border="false" square>
      <van-grid-item
        ...
        v-for="(channel, index) in shopInfos.channel.slice(0, 8)"
        :key="index"
      >
        <van-image round fit="contain" width="3rem" height="3rem" :src="channel.iconUrl" />
...
export default {
  ...
  methods: {
    goto(id){
      this.$router.replace(`/items/brand/${id}`);
    },
    getCode(event){
      this.addr=event[2].name;
      this.show=!this.show;
    },
    open(){
this.show=!this.show;
